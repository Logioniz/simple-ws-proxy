...
def parse_args() -> argparse.Namespace:
    ...
    Returns:
        ...
        - ``client_password`` (str | None): SOCKS5 password that clients must supply,
          or ``None`` to allow unauthenticated connections.
    """
    ...
    parser.add_argument(
        '--client-user',
        default=None,
        help='SOCKS5 username that connecting clients must provide (omit to allow no-auth)',
    )
    parser.add_argument(
        '--client-password',
        default=None,
        help='SOCKS5 password that connecting clients must provide (omit to allow no-auth)',
    )
